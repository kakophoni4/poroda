...
/** Формат «YYYYMMDD-HH», согласован с backfill-миграцией (тот же календарный час в stored timestamp). */
function hourBucketFromDate(d: Date = new Date()): string {
  return (
...
export async function POST(request: NextRequest) {
  ...
  if (csrf) return csrf;
  const hourBucket = hourBucketFromDate();
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      await tx.wheelSpinLog.create({
        data: { emailNorm, phoneNorm, hourBucket },
      });
    ...
    if (isUniqueConstraintError(e)) {
      return NextResponse.json({ error: "Вы уже крутили колесо в этом часу" }, { status: 429 });
    }
