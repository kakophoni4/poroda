  ...
  return (
    <div className="mt-6 space-y-8">
      ...
      {data.productStats && data.productStats.length > 0 && (
        <div className="rounded-2xl border border-zinc-200 p-6">
          <h3 className="font-medium">Аналитика по каждой позиции</h3>
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-zinc-200 text-left text-zinc-600">
                  <th className="pb-2 pr-4">Продукция</th>
      ...
      <div className="rounded-2xl border border-zinc-200 p-6">
        <h3 className="font-medium">Топ просматриваемой продукции</h3>
        <ul className="mt-4 space-y-2">
          {data.topProducts.length === 0 ? (
            <li className="text-sm text-zinc-500">Пока нет данных. Переходы на карточки продукции записываются автоматически.</li>
          ) : (
            data.topProducts.map((row) => (
              <li key={row.productId || row.viewCount} className="flex justify-between text-sm">
                <span>{row.product?.title ?? "Позиция удалена"}</span>
